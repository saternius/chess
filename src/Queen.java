...
public ArrayList<Square> posMove(int xPos, int yPos, boolean color) {
		...
		if(xPos+k<8 && !ChessBoard.noPeices(xPos+k, yPos) && white!=ChessBoard.board[xPos+k][yPos].peice.white){
			...
			if(ChessBoard.board[xPos+k][yPos].peice == ChessBoard.bKing){
				ChessBoard.bKing.attackers.add(this);
			}
			if(ChessBoard.board[xPos+k][yPos].peice == ChessBoard.wKing){
				ChessBoard.wKing.attackers.add(this);
			}
		}
		...
		if(yPos-k>-1 && !ChessBoard.noPeices(xPos, yPos-k) && white!=ChessBoard.board[xPos][yPos-k].peice.white){
			...
			if(ChessBoard.board[xPos][yPos-k].peice == ChessBoard.bKing){
				ChessBoard.bKing.attackers.add(this);
			}
			if(ChessBoard.board[xPos][yPos-k].peice == ChessBoard.wKing){
				ChessBoard.wKing.attackers.add(this);
			}
		}
		...
		if(xPos+k<8 && yPos+k<8 && !ChessBoard.noPeices(xPos+k, yPos+k) && white!=ChessBoard.board[xPos+k][yPos+k].peice.white){
			Integer[] pos = {xPos+k,yPos+k};
			ret.add(pos);
			if(ChessBoard.board[xPos+k][yPos+k].peice == ChessBoard.bKing){
				ChessBoard.bKing.attackers.add(this);
			}
			if(ChessBoard.board[xPos+k][yPos+k].peice == ChessBoard.wKing){
				ChessBoard.wKing.attackers.add(this);
			}
		}
